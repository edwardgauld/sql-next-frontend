...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case "SET_SKILLS":
            localStorage.setItem(SKILLS_LOCAL_STORAGE_ID, JSON.stringify(action.skills));
...
const page = () => {
    const [state, dispatch] = useReducer(reducer, {
        content: typeof window !== 'undefined' ? localStorage.getItem(CONTENT_LOCAL_STORAGE_ID) ?? "" : "",
        solution: typeof window !== 'undefined' ? localStorage.getItem(SOLUTION_LOCAL_STORAGE_ID) ?? "" : "",
        skills: typeof window !== 'undefined' ? JSON.parse(localStorage.getItem(SKILLS_LOCAL_STORAGE_ID) ?? "[]") : [],
    });
    ...
    const handleSubmit = async () => {
        try {
            ...
            alert(error.message || "An error occurred while creating the question");
        }
    }
  ...
  return (
    <div className='m-1'>
      ...
      <CodeEditor handleChange={handleSolutionChange} content={state.solution}/>
